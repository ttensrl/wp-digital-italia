...
if ( ! class_exists( 'CMB2_Bootstrap_2120_Develop', false ) ) {
	...
	class CMB2_Bootstrap_2120_Develop {
		...
		public function l10ni18n() {
			...
			$loaded = load_plugin_textdomain( 'wp-digital-italia', false, '/languages/' );

			if ( ! $loaded ) {
				$loaded = load_muplugin_textdomain( 'wp-digital-italia', '/languages/' );
			}

			if ( ! $loaded ) {
				$loaded = load_theme_textdomain( 'wp-digital-italia', get_stylesheet_directory() . '/languages/' );
			}

			if ( ! $loaded ) {
				$locale = apply_filters( 'plugin_locale', function_exists( 'determine_locale' ) ? determine_locale() : get_locale(), 'wp-digital-italia' );
				$mofile = dirname( __FILE__ ) . '/languages/cmb2-' . $locale . '.mo';
				load_textdomain( 'wp-digital-italia', $mofile );
			}
