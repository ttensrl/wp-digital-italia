...
class WDS_CMB2_Attached_Posts_Field {
    ...
    public function render( $field, $escaped_value, $object_id, $object_type, $field_type ) {
        ...
        if ( ! $query_users ) {
            ...
            $post_type_labels = $field_type->_text( 'users_text', esc_html__( 'Users', 'wp-digital-italia' ) );
        }

        $filter_boxes = '';
        // Check 'filter' setting
        if ( $this->field->options( 'filter_boxes' ) ) {
            $filter_boxes = '<div class="search-wrap"><input type="text" placeholder="' . sprintf( __( 'Filter %s', 'wp-digital-italia' ), $post_type_labels ) . '" class="regular-text search" name="%s" /></div>';
        }
        ...
        echo '<h4 class="attached-posts-section">' . sprintf( __( 'Available %s', 'wp-digital-italia' ), $post_type_labels ) . '</h4>';
        ...
        if ( ! $query_users ) {
            $findtxt = $field_type->_text( 'find_text', __( 'Search', 'wp-digital-italia' ) );

            $js_data = json_encode( array(
                'queryUsers' => $query_users,
                'types'      => $query_users ? 'user' : (array) $args['post_type'],
                'cmbId'      => $this->field->cmb_id,
                'errortxt'   => esc_attr( $field_type->_text( 'error_text', __( 'An error has occurred. Please reload the page and try again.', 'wp-digital-italia' ) ) ),
                'findtxt'    => esc_attr( $field_type->_text( 'find_text', __( 'Find Posts or Pages', 'wp-digital-italia' ) ) ),
        ...
        echo '<h4 class="attached-posts-section">' . sprintf( __( 'Attached %s', 'wp-digital-italia' ), $post_type_labels ) . '<span class="attached-posts-remaining hidden">(<span class="attached-posts-remaining-number"></span> remaining)</span></span></h4>';
    ...
    public function list_item( $object, $li_class, $icon_class = 'dashicons-plus' ) {
        // Build our list item
        printf(
            '<li data-id="%1$d" class="%2$s" target="_blank">%3$s<a title="' . __( 'Edit', 'wp-digital-italia' ) . '" href="%4$s">%5$s</a>%6$s<span class="dashicons %7$s add-remove"></span></li>',
            $this->get_id( $object ),
            $li_class,
