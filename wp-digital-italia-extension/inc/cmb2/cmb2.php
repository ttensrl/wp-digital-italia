...
require 'plugins/CMB2-conditional-logic/cmb2-conditional-logic.php';
require 'plugins/CMB2-field-Leaflet-Geocoder/cmb-field-leaflet-map.php';
require 'plugins/cmb2-attached-posts/cmb2-attached-posts-field.php';
require 'plugins/CMB2-taxonomy-hierarchy-child.php';
 ...
 * @return array
 */
function dci_get_user_options( $query_args = false): array
{
...
function dci_get_posts_options( $post_type = 'post', $parent = false, $addnone=false) {
    ...
    $options = array();
    if($addnone)
        $options[0]=__('Nessun contenuto','wp-digital-italia');
    if ( $posts ) {
...
function dci_get_first_level_pages_options($addnone=false) {
    ...
    $options = array();
    if($addnone)
        $options[0]=__('Nessuna pagina','wp-digital-italia');
    if ( $pages ) {
